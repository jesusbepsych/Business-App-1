(() => {
  ...
  function analyticsExpenseCategoryChart(buckets,expenses) {
    ...
    const frame=analyticsChartFrame(buckets,totals,{height:330});const baseline=frame.y(0);const slot=frame.innerW/Math.max(1,buckets.length);const barWidth=Math.min(72,Math.max(34,slot*.6));
    ...
    const columnClipDefs=buckets.map((bucket,index)=>{const columnTop=totals[index]>0?frame.y(totals[index]):baseline;const columnHeight=Math.max(0,baseline-columnTop);return `<clipPath id="categoryColumnClip${index}"><rect x="${frame.x(index)-barWidth/2}" y="${columnTop}" width="${barWidth}" height="${columnHeight}" rx="6"/></clipPath>`}).join('');
    const bars=buckets.map((bucket,index)=>{let cumulative=0;
      const segments=series.map((item,seriesIndex)=>{const value=monthValues[index].get(item.key)||0;if(value<=0)return '';const start=cumulative,end=cumulative+value; cumulative=end;const top=frame.y(end),bottom=frame.y(start),height=bottom-top;return `<rect class="category-column-segment" data-category="${escapeHtml(item.key)}" data-segment-cents="${value}" x="${frame.x(index)-barWidth/2}" y="${top}" width="${barWidth}" height="${Math.max(.5,height)}" fill="url(#categoryColumnFill${seriesIndex})" style="--category-color:${item.color}"><title>${escapeHtml(bucket.label)} · ${escapeHtml(item.label)} · ${formatMoney(value)}</title></rect>`}).join('');
      ...
      return `<g class="stacked-month category-month" data-chart-index="${index}" data-chart-x="${frame.x(index)}" data-chart-y="${frame.y(totals[index])}" data-chart-value="${totals[index]}" data-chart-label="${escapeHtml(bucket.label)}">${totals[index]>0?`<rect class="category-column-aura" x="${frame.x(index)-barWidth/2-6}" y="${columnTop-5}" width="${barWidth+12}" height="${columnHeight+10}" rx="11"/><g class="category-column-fill" clip-path="url(#categoryColumnClip${index})">${segments}</g><rect class="category-column-frame" x="${frame.x(index)-barWidth/2}" y="${columnTop}" width="${barWidth}" height="${columnHeight}" rx="6"/><rect class="category-column-sheen" x="${frame.x(index)-barWidth/2+1.5}" y="${columnTop+1.5}" width="${barWidth-3}" height="${Math.max(1,columnHeight-3)}" rx="4.75"/>`:''}${totals[index]>0?`<text class="category-column-total" x="${frame.x(index)}" y="${totalY}" text-anchor="middle">${formatMoney(totals[index])}</text>`:''}<rect class="stacked-hit" x="${frame.x(index)-slot/2}" y="${frame.top}" width="${slot}" height="${frame.innerH}"/>${index%frame.labelEvery===0||index===buckets.length-1?`<text class="category-month-label" x="${frame.x(index)}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}</g>`}).join('');
    const legend=series.map(item=>`<span><i style="--category-color:${item.color}"></i>${escapeHtml(item.label)}</span>`).join('');
    return `<svg class="analytics-svg category-stack-svg category-column-chart scrubbable-chart" data-scrub-mode="stacked" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Business-use expenses shown as continuous monthly columns stacked by category. Touch and drag to inspect monthly totals."><defs>${gradientDefs}${columnClipDefs}</defs>${frame.grid}${bars}<g class="chart-scrubber stacked-scrubber" aria-hidden="true"><line x1="0" x2="0" y1="${frame.top}" y2="${frame.top+frame.innerH}"/><circle class="expense" cx="0" cy="${baseline}" r="5.5"/><g class="scrub-tooltip"><rect x="-54" y="-42" width="108" height="32" rx="9"/><text class="scrub-label" x="0" y="-29" text-anchor="middle"></text><text class="scrub-value" x="0" y="-17" text-anchor="middle"></text></g></g></svg><div class="category-stack-legend" aria-label="Expense categories">${legend}</div>`;
  }
