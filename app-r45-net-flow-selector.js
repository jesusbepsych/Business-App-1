(() => {
  ...
  function analyticsNetFlowChart(buckets) {
    ...
    const values=buckets.map(x=>x.received-expenseAmount(x)),frame=analyticsChartFrame(buckets,values),points=values.map((value,i)=>({x:frame.x(i),y:frame.y(value),value})),path=smoothChartPath(points),baseline=frame.y(0);
  ...
  function renderAnalytics() {
    ...
    const netBasisLabels={business:'Received minus business expenses',all:'Received minus all expenses'};
