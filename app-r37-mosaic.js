(() => {
  ...
  function analyticsExpenseCategoryChart(buckets,expenses) {
    ...
    const totals=monthValues.map(values=>[...values.values()].reduce((n,value)=>n+value,0));const largestTotal=Math.max(0,...totals);const blockScaleCeiling=Math.max(MOSAIC_BLOCK_CENTS,Math.ceil(largestTotal/MOSAIC_BLOCK_CENTS)*MOSAIC_BLOCK_CENTS);const frame=analyticsChartFrame(buckets,[...totals,blockScaleCeiling],{height:285});const baseline=frame.y(0);const slot=frame.innerW/Math.max(1,buckets.length);const barWidth=Math.min(54,Math.max(20,slot*.55));
    const bars=buckets.map((bucket,index)=>{let cumulative=0;const fragments=series.map(item=>{let remaining=monthValues[index].get(item.key)||0;if(!remaining)return '';const categoryValue=remaining;const pieces=[];while(remaining>0){const offset=cumulative%MOSAIC_BLOCK_CENTS,capacity=offset===0?MOSAIC_BLOCK_CENTS:MOSAIC_BLOCK_CENTS-offset,chunk=Math.min(capacity,remaining),start=cumulative,end=cumulative+chunk,rawY=frame.y(end),rawHeight=Math.max(1,frame.y(start)-rawY),inset=Math.min(1.35,Math.max(.18,rawHeight*.1));pieces.push(`<rect class="mosaic-fragment" data-fragment-cents="${chunk}" x="${frame.x(index)-barWidth/2+1.5}" y="${rawY+inset/2}" width="${barWidth-3}" height="${Math.max(.6,rawHeight-inset)}" rx="2.2" fill="${item.color}" style="--mosaic-color:${item.color}"><title>${escapeHtml(bucket.label)} · ${escapeHtml(item.label)} · ${formatMoney(chunk)}</title></rect>`);cumulative=end;remaining-=chunk}return `<g class="mosaic-category" data-category="${escapeHtml(item.key)}"><title>${escapeHtml(bucket.label)} · ${escapeHtml(item.label)} total · ${formatMoney(categoryValue)}</title>${pieces.join('')}</g>`}).join('');const unitCount=Math.ceil(totals[index]/MOSAIC_BLOCK_CENTS);const shells=Array.from({length:unitCount},(_,unitIndex)=>{const start=unitIndex*MOSAIC_BLOCK_CENTS,end=start+MOSAIC_BLOCK_CENTS,rawY=frame.y(end),rawHeight=frame.y(start)-rawY;return `<rect class="mosaic-unit-shell" data-unit-cents="${MOSAIC_BLOCK_CENTS}" x="${frame.x(index)-barWidth/2}" y="${rawY+.8}" width="${barWidth}" height="${Math.max(1,rawHeight-1.6)}" rx="3.4"><title>${escapeHtml(bucket.label)} · $${(unitIndex*250).toLocaleString('en-US')}–$${((unitIndex+1)*250).toLocaleString('en-US')} unit</title></rect>`}).join('');const totalY=Math.max(12,frame.y(totals[index])-6);return `<g class="stacked-month mosaic-month" data-chart-index="${index}" data-chart-x="${frame.x(index)}" data-chart-y="${frame.y(totals[index])}" data-chart-value="${totals[index]}" data-chart-label="${escapeHtml(bucket.label)}">${fragments}${shells}${totals[index]>0?`<text class="mosaic-total" x="${frame.x(index)}" y="${totalY}" text-anchor="middle">${formatMoney(totals[index])}</text>`:''}<rect class="stacked-hit" x="${frame.x(index)-slot/2}" y="${frame.top}" width="${slot}" height="${frame.innerH}"/>${index%frame.labelEvery===0||index===buckets.length-1?`<text class="mosaic-month-label" x="${frame.x(index)}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}</g>`}).join('');
    const legend=series.map(item=>`<span><i style="--category-color:${item.color}"></i>${escapeHtml(item.label)}</span>`).join('');
    return `<svg class="analytics-svg category-stack-svg mosaic-chart-svg scrubbable-chart" data-scrub-mode="stacked" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Business-use expenses shown as measured mosaic columns. Each full block represents $250; partial blocks preserve exact amounts. Touch and drag to inspect monthly totals.">${frame.grid}${bars}<g class="chart-scrubber stacked-scrubber" aria-hidden="true"><line x1="0" x2="0" y1="${frame.top}" y2="${frame.top+frame.innerH}"/><circle class="expense" cx="0" cy="${baseline}" r="5.5"/><g class="scrub-tooltip"><rect x="-54" y="-42" width="108" height="32" rx="9"/><text class="scrub-label" x="0" y="-29" text-anchor="middle"></text><text class="scrub-value" x="0" y="-17" text-anchor="middle"></text></g></g></svg><div class="category-stack-legend" aria-label="Expense categories">${legend}</div>`;
  }
