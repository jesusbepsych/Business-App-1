(() => {
  ...
  function analyticsHalfMonthBuckets(bounds) {
    const cursor=parseDateOnlyUtc(`${bounds.start.slice(0,7)}-01`);const endMonth=parseDateOnlyUtc(`${bounds.end.slice(0,7)}-01`);const out=[];const includeYear=bounds.start.slice(0,4)!==bounds.end.slice(0,4);
    while(cursor<=endMonth){
      const year=cursor.getUTCFullYear(),monthIndex=cursor.getUTCMonth(),month=String(monthIndex+1).padStart(2,'0'),monthKey=`${year}-${month}`,monthEnd=new Date(Date.UTC(year,monthIndex+1,0)).getUTCDate();
      const monthLabel=new Intl.DateTimeFormat('en-US',{month:'short',year:includeYear?'2-digit':undefined,timeZone:'UTC'}).format(cursor);
      [[1,15],[16,monthEnd]].forEach(([startDay,endDay],halfIndex)=>{
        const start=`${monthKey}-${String(startDay).padStart(2,'0')}`,end=`${monthKey}-${String(endDay).padStart(2,'0')}`;
        if(end<bounds.start||start>bounds.end)return;
        const effectiveStart=start<bounds.start?bounds.start:start,effectiveEnd=end>bounds.end?bounds.end:end;
        out.push({key:`${monthKey}-${halfIndex+1}`,monthKey,start:effectiveStart,end:effectiveEnd,label:`${monthLabel} ${Number(effectiveStart.slice(-2))}–${Number(effectiveEnd.slice(-2))}`,received:0,expenses:0,allExpenses:0,hours:0,uninvoiced:0});
      });
      cursor.setUTCMonth(cursor.getUTCMonth()+1);
    }
    const bucketFor=date=>out.find(bucket=>dateInBounds(date,bucket.start,bucket.end));
    analyticsPaymentsForReview().filter(x=>dateInBounds(x.receivedDate,bounds.start,bounds.end)).forEach(x=>{const bucket=bucketFor(x.receivedDate);if(bucket)bucket.received+=Number(x.amountCents||0)});
    analyticsExpensesForReview().filter(x=>dateInBounds(x.date,bounds.start,bounds.end)).forEach(x=>{const bucket=bucketFor(x.date);if(bucket){bucket.expenses+=Number(x.businessCents||0);bucket.allExpenses+=Number(x.totalCents||0)}});
    analyticsSessionsForReview().filter(x=>dateInBounds(x.date,bounds.start,bounds.end)).forEach(x=>{const bucket=bucketFor(x.date);if(bucket){bucket.hours+=sessionMinutes(x);if(x.invoiceStatus==='uninvoiced')bucket.uninvoiced+=sessionAmountCents(x)}});
    return out;
  }
  ...
  function analyticsChartFrame(buckets,values,{height=250,width=760}={}) {
    const left=66,right=16,top=18,bottom=40,innerW=width-left-right,innerH=height-top-bottom,domain=niceMoneyDomain(values);
  ...
  function analyticsNetFlowChart(buckets,{height=360,width=760,periodLabel='month'}={}) {
    ...
    const values=buckets.map(x=>x.received-expenseAmount(x)),frame=analyticsChartFrame(buckets,values,{height,width}),points=values.map((value,i)=>({x:frame.x(i),y:frame.y(value),value})),path=smoothChartPath(points),baseline=frame.y(0);
    const area=`${path} L ${points[points.length-1].x} ${baseline} L ${points[0].x} ${baseline} Z`;
    return `<svg class="analytics-svg scrubbable-chart" data-scrub-mode="net" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Net movement with a proportional money axis. Touch and drag to inspect each ${periodLabel}."><defs><linearGradient id="netFlowFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#72deb0" stop-opacity=".25"/><stop offset="1" stop-color="#72deb0" stop-opacity="0"/></linearGradient></defs>${frame.grid}<path class="chart-area net" d="${area}"/><path class="chart-line net" d="${path}"/>${buckets.map((bucket,i)=>`<g class="chart-hit net-hit" data-chart-index="${i}" data-chart-x="${points[i].x}" data-chart-y="${points[i].y}" data-chart-value="${points[i].value}" data-chart-label="${escapeHtml(bucket.label)}"><rect x="${Math.max(frame.left,points[i].x-Math.max(12,frame.innerW/Math.max(2,buckets.length)/2))}" y="${frame.top}" width="${Math.max(24,frame.innerW/Math.max(1,buckets.length))}" height="${frame.innerH}"/>${i%frame.labelEvery===0||i===buckets.length-1?`<text x="${points[i].x}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}<title>${escapeHtml(bucket.label)} · ${formatMoney(points[i].value)} net movement</title></g>`).join('')}<g class="chart-scrubber" aria-hidden="true"><line x1="0" x2="0" y1="${frame.top}" y2="${frame.top+frame.innerH}"/><circle class="net" cx="0" cy="0" r="6"/><g class="scrub-tooltip"><rect x="-64" y="-42" width="128" height="32" rx="9"/><text class="scrub-label" x="0" y="-29" text-anchor="middle"></text><text class="scrub-value" x="0" y="-17" text-anchor="middle"></text></g></g></svg>`;
  }
  ...
  function openExpandedCashflow(bounds=analyticsRangeBounds()) {
    const buckets=analyticsHalfMonthBuckets(bounds);const chartWidth=Math.max(1040,Math.min(1800,buckets.length*82));
    $('#analyticsCashflowExpandedRange').textContent=`${formatDate(bounds.start,{month:'short',day:'numeric',year:'numeric'})} – ${formatDate(bounds.end,{month:'short',day:'numeric',year:'numeric'})} · two periods per month`;
    $('#analyticsCashflowExpandedBasis').textContent=ui.analyticsNetExpenseBasis==='all'?'Received minus all expenses':'Received minus business expenses';
    const chartHost=$('#analyticsCashflowExpandedChart');
    chartHost.innerHTML=analyticsNetFlowChart(buckets,{height:500,width:chartWidth,periodLabel:'half-month period'});
    $('svg',chartHost)?.style.setProperty('min-width',`${chartWidth}px`);
    closeFilterMenu();
    openModal($('#analyticsCashflowExpanded'));
    bindAnalyticsScrubber(chartHost);
  }
  ...
  function bindAnalyticsInteractions(bounds) {
    ...
    const expandButton=$('#analyticsCashflowExpandBtn');
    if(expandButton)expandButton.onclick=()=>openExpandedCashflow(bounds);
  ...
  function paymentNeedsAttention(payment) {
    const explicitState=String(payment.reviewStatus||payment.verificationStatus||payment.status||'').toLowerCase();
    if(['needs_review','unverified','draft'].includes(explicitState))return true;
    return !payment.receivedDate||Number(payment.amountCents||0)<=0||!payment.kind;
  }
  ...
  function renderMoney() {
    ...
    $('#invoiceCount').textContent = invoices.filter(invoice => invoice.status === 'draft').length;
    $('#paymentCount').textContent = payments.filter(paymentNeedsAttention).length;
    $('#expenseCount').textContent = expenses.filter(expense => expense.reviewStatus === 'needs_review').length;
