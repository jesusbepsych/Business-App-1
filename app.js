(() => {
  ...
  function analyticsExpenseCategoryChart(buckets,expenses) {
    ...
    const totals=monthValues.map(values=>[...values.values()].reduce((n,value)=>n+value,0)); const frame=analyticsChartFrame(buckets,totals,{height:285}); const baseline=frame.y(0); const slot=frame.innerW/Math.max(1,buckets.length); const barWidth=Math.min(54,Math.max(20,slot*.55));
    const bars=buckets.map((bucket,index)=>{let cumulative=0;const stacks=series.map(item=>{let remaining=monthValues[index].get(item.key)||0;if(!remaining)return '';const categoryValue=remaining;const blocks=[];while(remaining>0){const chunk=Math.min(MOSAIC_BLOCK_CENTS,remaining),start=cumulative,end=cumulative+chunk,rawY=frame.y(end),rawHeight=Math.max(1,frame.y(start)-rawY),gap=Math.min(2,Math.max(.45,rawHeight*.16));blocks.push(`<rect class="mosaic-block" data-block-cents="${chunk}" x="${frame.x(index)-barWidth/2}" y="${rawY+gap/2}" width="${barWidth}" height="${Math.max(.75,rawHeight-gap)}" rx="3" fill="${item.color}" style="--mosaic-color:${item.color}"><title>${escapeHtml(bucket.label)} · ${escapeHtml(item.label)} · ${formatMoney(chunk)}${chunk<MOSAIC_BLOCK_CENTS?' partial block':' block'}</title></rect>`);cumulative=end;remaining-=chunk}return `<g class="mosaic-category" data-category="${escapeHtml(item.key)}"><title>${escapeHtml(bucket.label)} · ${escapeHtml(item.label)} total · ${formatMoney(categoryValue)}</title>${blocks.join('')}</g>`}).join('');const totalY=Math.max(12,frame.y(totals[index])-7);return `<g class="stacked-month mosaic-month" data-chart-index="${index}" data-chart-x="${frame.x(index)}" data-chart-y="${frame.y(totals[index])}" data-chart-value="${totals[index]}" data-chart-label="${escapeHtml(bucket.label)}">${stacks}${totals[index]>0?`<text class="mosaic-total" x="${frame.x(index)}" y="${totalY}" text-anchor="middle">${formatMoney(totals[index])}</text>`:''}<rect class="stacked-hit" x="${frame.x(index)-slot/2}" y="${frame.top}" width="${slot}" height="${frame.innerH}"/>${index%frame.labelEvery===0||index===buckets.length-1?`<text class="mosaic-month-label" x="${frame.x(index)}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}</g>`}).join('');
