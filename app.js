(() => {
  ...
  const ANALYTICS_PIE_COLORS=['#72deb0','#75bfe7','#a991ee','#efb66c','#ed8fae','#8fd0c5','#d9c56e','#7da9ef','#cf91c9','#91c77d'];
  ...
  function analyticsExpenseCategoryChart(buckets,expenses) {
    ...
    const ranked=[...categoryTotals].sort((a,b)=>b[1]-a[1]);
    ...
    const series=leading.map(([key,total])=>({key,label:expenseCategoryLabel(key),total,color:ANALYTICS_PIE_COLORS[leading.findIndex(item=>item[0]===key)%ANALYTICS_PIE_COLORS.length]}));
    ...
    const totals=monthValues.map(values=>[...values.values()].reduce((n,value)=>n+value,0)); const frame=analyticsChartFrame(buckets,totals,{height:265}); const baseline=frame.y(0); const slot=frame.innerW/Math.max(1,buckets.length); const barWidth=Math.min(56,Math.max(18,slot*.58));
    const bars=buckets.map((bucket,index)=>{let cumulative=0;const stacks=series.map(item=>{const value=monthValues[index].get(item.key)||0;if(!value)return '';const start=cumulative;cumulative+=value;const y=frame.y(cumulative),height=Math.max(1,frame.y(start)-y);return `<rect class="category-stack" x="${frame.x(index)-barWidth/2}" y="${y}" width="${barWidth}" height="${height}" rx="${cumulative===totals[index]?4:0}" fill="${item.color}"><title>${escapeHtml(bucket.label)} · ${escapeHtml(item.label)} · ${formatMoney(value)}</title></rect>`}).join('');return `<g class="stacked-month" data-chart-index="${index}" data-chart-x="${frame.x(index)}" data-chart-y="${frame.y(totals[index])}" data-chart-value="${totals[index]}" data-chart-label="${escapeHtml(bucket.label)}">${stacks}<rect class="stacked-hit" x="${frame.x(index)-slot/2}" y="${frame.top}" width="${slot}" height="${frame.innerH}"/>${index%frame.labelEvery===0||index===buckets.length-1?`<text x="${frame.x(index)}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}</g>`}).join('');
    const legend=series.map(item=>`<span><i style="--category-color:${item.color}"></i>${escapeHtml(item.label)}</span>`).join('');
    return `<svg class="analytics-svg category-stack-svg scrubbable-chart" data-scrub-mode="stacked" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Business-use expenses stacked by category and month. Touch and drag to inspect monthly totals.">${frame.grid}${bars}<g class="chart-scrubber stacked-scrubber" aria-hidden="true"><line x1="0" x2="0" y1="${frame.top}" y2="${frame.top+frame.innerH}"/><circle class="expense" cx="0" cy="${baseline}" r="5.5"/><g class="scrub-tooltip"><rect x="-54" y="-42" width="108" height="32" rx="9"/><text class="scrub-label" x="0" y="-29" text-anchor="middle"></text><text class="scrub-value" x="0" y="-17" text-anchor="middle"></text></g></g></svg><div class="category-stack-legend" aria-label="Expense categories">${legend}</div>`;
  }
  ...
  function renderAnalytics() {
    ...
    const clients=[...clientMap.values()].sort((a,b)=>b.cents-a.cents||b.minutes-a.minutes); const maxClient=Math.max(1,...clients.map(x=>x.cents));const clientPages=Math.max(1,Math.ceil(clients.length/ui.analyticsPageSize));ui.analyticsClientPage=Math.min(clientPages,Math.max(1,ui.analyticsClientPage));const clientStart=(ui.analyticsClientPage-1)*ui.analyticsPageSize;const visibleClients=clients.slice(clientStart,clientStart+ui.analyticsPageSize);
    $('#analyticsClients').innerHTML=clients.length?`${visibleClients.map((x,index)=>{const globalIndex=clientStart+index;return `<button class="analytics-rank-row" style="--slice-color:${ANALYTICS_PIE_COLORS[clients.length>5&&globalIndex>=5?5:globalIndex%ANALYTICS_PIE_COLORS.length]}" data-analytics-client="${escapeHtml(x.key)}"><span><strong>${escapeHtml(x.name)}</strong><small>${x.count} ${x.count===1?'payment':'payments'} · ${(x.minutes/60).toFixed(x.minutes%60?1:0)}h logged</small></span><span class="rank-track"><i style="--rank:${(x.cents/maxClient)*100}%"></i></span><strong>${formatMoney(x.cents,activeBusiness().currency)}</strong></button>`}).join('')}${analyticsPagination('client',ui.analyticsClientPage,clientPages,clients.length)}`:'<div class="analytics-empty"><strong>No client activity yet</strong><small>Income and workload appear from Payments and Work Sessions.</small></div>';
    $('#analyticsIncomePie').innerHTML=analyticsDonutChart(clients,'client');
    const categoryMap=new Map(); expenses.forEach(expense=>{const cents=Number(expense.businessCents||0);if(cents<=0)return;const row=categoryMap.get(expense.category)||{key:expense.category,label:expenseCategoryLabel(expense.category),cents:0,count:0};row.cents+=cents;row.count++;categoryMap.set(expense.category,row)});const categories=[...categoryMap.values()].sort((a,b)=>b.cents-a.cents);const maxCategory=Math.max(1,...categories.map(x=>x.cents));const categoryPages=Math.max(1,Math.ceil(categories.length/ui.analyticsPageSize));ui.analyticsExpensePage=Math.min(categoryPages,Math.max(1,ui.analyticsExpensePage));const categoryStart=(ui.analyticsExpensePage-1)*ui.analyticsPageSize;const visibleCategories=categories.slice(categoryStart,categoryStart+ui.analyticsPageSize);
    $('#analyticsExpenses').innerHTML=categories.length?`${visibleCategories.map((x,index)=>{const globalIndex=categoryStart+index;return `<button class="analytics-rank-row" style="--slice-color:${ANALYTICS_PIE_COLORS[categories.length>5&&globalIndex>=5?5:globalIndex%ANALYTICS_PIE_COLORS.length]}" data-analytics-category="${x.key}"><span><strong>${escapeHtml(x.label)}</strong><small>${x.count} ${x.count===1?'record':'records'}</small></span><span class="rank-track expense"><i style="--rank:${(x.cents/maxCategory)*100}%"></i></span><strong>${formatMoney(x.cents,activeBusiness().currency)}</strong></button>`}).join('')}${analyticsPagination('expense',ui.analyticsExpensePage,categoryPages,categories.length)}`:'<div class="analytics-empty"><strong>No business-use expenses</strong><small>Category patterns will appear as expenses are recorded.</small></div>';
  ...
  function bindAnalyticsScrubber(host) {
    ...
    const paint=()=>{frame=null;if(pendingX==null)return;const x=Math.max(points[0].x,Math.min(points[points.length-1].x,pendingX));let nearestIndex=0;for(let i=1;i<points.length;i++)if(Math.abs(points[i].x-x)<Math.abs(points[nearestIndex].x-x))nearestIndex=i;const nearest=points[nearestIndex];pulse(nearestIndex);
    ...
    svg.addEventListener('pointerup',event=>{tracking=false;hapticsActive=false;svg.releasePointerCapture?.(event.pointerId);if(event.pointerType!=='mouse')hideTimer=setTimeout(()=>scrubber.classList.remove('active'),1400);setTimeout(()=>{moved=false},0)});
    svg.addEventListener('pointercancel',()=>{tracking=false;hapticsActive=false;scrubber.classList.remove('active')});
  ...
  function bindAnalyticsInteractions(bounds) {
    ...
    bindAnalyticsScrubber($('#analyticsExpenseCategoryChart'));
  }
