(() => {
  ...
  function analyticsNetFlowChart(buckets) {
    if(!buckets.length||!buckets.some(x=>x.received||x.expenses))return '<div class="analytics-empty"><strong>No business activity in this range</strong><small>Net movement will appear from Payments and business-use Expenses.</small></div>';
    const values=buckets.map(x=>x.received-x.expenses),frame=analyticsChartFrame(buckets,values),points=values.map((value,i)=>({x:frame.x(i),y:frame.y(value),value})),path=smoothChartPath(points),baseline=frame.y(0);
    const area=`${path} L ${points[points.length-1].x} ${baseline} L ${points[0].x} ${baseline} Z`;
    return `<svg class="analytics-svg scrubbable-chart" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Monthly net movement with a proportional money axis"><defs><linearGradient id="netFlowFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#72deb0" stop-opacity=".25"/><stop offset="1" stop-color="#72deb0" stop-opacity="0"/></linearGradient></defs>${frame.grid}<path class="chart-area net" d="${area}"/><path class="chart-line net" d="${path}"/>${buckets.map((bucket,i)=>`<g class="chart-hit net-hit" tabindex="0" role="button" data-analytics-month="${bucket.key}" data-chart-index="${i}" data-chart-x="${points[i].x}" data-chart-y="${points[i].y}" data-chart-value="${points[i].value}" data-chart-label="${escapeHtml(bucket.label)}" aria-label="${escapeHtml(bucket.label)}: ${formatMoney(points[i].value)} net movement"><rect x="${Math.max(frame.left,points[i].x-Math.max(12,frame.innerW/Math.max(2,buckets.length)/2))}" y="${frame.top}" width="${Math.max(24,frame.innerW/Math.max(1,buckets.length))}" height="${frame.innerH}"/>${i%frame.labelEvery===0||i===buckets.length-1?`<text x="${points[i].x}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}<title>${escapeHtml(bucket.label)} · ${formatMoney(points[i].value)} net movement</title></g>`).join('')}<g class="chart-scrubber" aria-hidden="true"><line x1="0" x2="0" y1="${frame.top}" y2="${frame.top+frame.innerH}"/><circle class="net" cx="0" cy="0" r="6"/><g class="scrub-tooltip"><rect x="-54" y="-42" width="108" height="32" rx="9"/><text class="scrub-label" x="0" y="-29" text-anchor="middle"></text><text class="scrub-value" x="0" y="-17" text-anchor="middle"></text></g></g></svg>`;
  }
  ...
  function analyticsIncomeExpenseChart(buckets) {
    ...
    const frame=analyticsChartFrame(buckets,buckets.flatMap(x=>[x.received,x.expenses]),{height:230}); const income=buckets.map((x,i)=>({x:frame.x(i),y:frame.y(x.received)})),expense=buckets.map((x,i)=>({x:frame.x(i),y:frame.y(x.expenses)}));const incomePath=smoothChartPath(income),expensePath=smoothChartPath(expense),base=frame.y(0);
    ...
    return `<svg class="analytics-svg comparison-svg" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Income received versus business-use expenses by month"><defs><linearGradient id="incomeAreaFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#72deb0" stop-opacity=".20"/><stop offset="1" stop-color="#72deb0" stop-opacity="0"/></linearGradient><linearGradient id="expenseAreaFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ef9dba" stop-opacity=".18"/><stop offset="1" stop-color="#ef9dba" stop-opacity="0"/></linearGradient></defs>${frame.grid}<path class="chart-area income" d="${incomeArea}"/><path class="chart-area expense" d="${expenseArea}"/><path class="chart-line income" d="${incomePath}"/><path class="chart-line expense" d="${expensePath}"/>${buckets.map((bucket,i)=>`<g class="chart-hit" tabindex="0" role="button" data-analytics-month="${bucket.key}" aria-label="${escapeHtml(bucket.label)}: ${formatMoney(bucket.received)} income, ${formatMoney(bucket.expenses)} expenses"><rect x="${Math.max(frame.left,income[i].x-Math.max(12,frame.innerW/Math.max(2,buckets.length)/2))}" y="${frame.top}" width="${Math.max(24,frame.innerW/Math.max(1,buckets.length))}" height="${frame.innerH}"/><circle class="income" cx="${income[i].x}" cy="${income[i].y}" r="3.5"/><circle class="expense" cx="${expense[i].x}" cy="${expense[i].y}" r="3.5"/>${i%frame.labelEvery===0||i===buckets.length-1?`<text x="${income[i].x}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}<title>${escapeHtml(bucket.label)} · ${formatMoney(bucket.received)} income · ${formatMoney(bucket.expenses)} expenses</title></g>`).join('')}</svg>`;
  }
  ...
  const ANALYTICS_PIE_COLORS=['#72deb0','#75bfe7','#a991ee','#efb66c','#ed8fae','#8fd0c5'];

  function analyticsDonutSlices(items) {
    const positive=items.filter(item=>Number(item.cents||0)>0).sort((a,b)=>b.cents-a.cents);
    if(positive.length<=5)return positive.map(item=>({...item,keys:[item.key]}));
    const leading=positive.slice(0,5); const remainder=positive.slice(5);
    return [...leading.map(item=>({...item,keys:[item.key]})),{key:'other',label:'Other',name:'Other',cents:remainder.reduce((n,x)=>n+x.cents,0),keys:remainder.map(x=>x.key)}];
  }

  function analyticsDonutChart(items,kind) {
    const slices=analyticsDonutSlices(items),total=slices.reduce((n,x)=>n+x.cents,0);
    if(!total)return '<div class="donut-empty"><span></span><small>No data</small></div>';
    let offset=0;
    const rings=slices.map((slice,index)=>{const percent=slice.cents/total*100;const dash=`${percent} ${100-percent}`;const encoded=encodeURIComponent(JSON.stringify(slice.keys));const label=slice.name||slice.label||'Source';const out=`<circle class="donut-slice" tabindex="0" role="button" data-analytics-pie-kind="${kind}" data-analytics-pie-keys="${encoded}" cx="50" cy="50" r="38" pathLength="100" stroke="${ANALYTICS_PIE_COLORS[index%ANALYTICS_PIE_COLORS.length]}" stroke-dasharray="${dash}" stroke-dashoffset="${-offset}" aria-label="${escapeHtml(label)}: ${formatMoney(slice.cents)}, ${Math.round(percent)} percent"><title>${escapeHtml(label)} · ${formatMoney(slice.cents)} · ${Math.round(percent)}%</title></circle>`;offset+=percent;return out}).join('');
    return `<svg viewBox="0 0 100 100" class="donut-svg" role="img" aria-label="${kind==='client'?'Income by source':'Business-use expenses by category'}"><circle class="donut-track" cx="50" cy="50" r="38" pathLength="100"/>${rings}<circle class="donut-hole" cx="50" cy="50" r="28"/><text class="donut-total" x="50" y="48" text-anchor="middle">${moneyAxisLabel(total)}</text><text class="donut-caption" x="50" y="58" text-anchor="middle">total</text></svg>`;
  }
  ...
  function renderAnalytics() {
    ...
    const buckets=analyticsMonthBuckets(bounds); $('#analyticsCashflowChart').innerHTML=analyticsNetFlowChart(buckets); $('#analyticsIncomeExpenseChart').innerHTML=analyticsIncomeExpenseChart(buckets);
    ...
    $('#analyticsClients').innerHTML=clients.length?clients.map((x,index)=>`<button class="analytics-rank-row" style="--slice-color:${ANALYTICS_PIE_COLORS[clients.length>5&&index>=5?5:index%ANALYTICS_PIE_COLORS.length]}" data-analytics-client="${escapeHtml(x.key)}"><span><strong>${escapeHtml(x.name)}</strong><small>${x.count} ${x.count===1?'payment':'payments'} · ${(x.minutes/60).toFixed(x.minutes%60?1:0)}h logged</small></span><span class="rank-track"><i style="--rank:${(x.cents/maxClient)*100}%"></i></span><strong>${formatMoney(x.cents,activeBusiness().currency)}</strong></button>`).join(''):'<div class="analytics-empty"><strong>No client activity yet</strong><small>Income and workload appear from Payments and Work Sessions.</small></div>';
    $('#analyticsIncomePie').innerHTML=analyticsDonutChart(clients,'client');
    ...
    $('#analyticsExpenses').innerHTML=categories.length?categories.map((x,index)=>`<button class="analytics-rank-row" style="--slice-color:${ANALYTICS_PIE_COLORS[categories.length>5&&index>=5?5:index%ANALYTICS_PIE_COLORS.length]}" data-analytics-category="${x.key}"><span><strong>${escapeHtml(x.label)}</strong><small>${x.count} ${x.count===1?'record':'records'}</small></span><span class="rank-track expense"><i style="--rank:${(x.cents/maxCategory)*100}%"></i></span><strong>${formatMoney(x.cents,activeBusiness().currency)}</strong></button>`).join(''):'<div class="analytics-empty"><strong>No business-use expenses</strong><small>Category patterns will appear as expenses are recorded.</small></div>';
    $('#analyticsExpensePie').innerHTML=analyticsDonutChart(categories,'category');
  ...
  function bindAnalyticsScrubber(host) {
    ...
    const pointNodes=$$('[data-chart-index]',svg); const scrubber=$('.chart-scrubber',svg); const path=$('.chart-line.net',svg); if(!pointNodes.length||!scrubber||!path)return;
    const points=pointNodes.map(node=>({x:Number(node.dataset.chartX),y:Number(node.dataset.chartY),value:Number(node.dataset.chartValue),label:node.dataset.chartLabel}));
    const guide=$('line',scrubber),dot=$('circle',scrubber),tooltip=$('.scrub-tooltip',scrubber),label=$('.scrub-label',scrubber),value=$('.scrub-value',scrubber); let tracking=false,pendingX=null,frame=null,hideTimer=null,startClientX=0,moved=false;
    const curvePointForX=x=>{if(!path.getTotalLength||!path.getPointAtLength){const nearest=points.reduce((best,p)=>Math.abs(p.x-x)<Math.abs(best.x-x)?p:best,points[0]);return{x,y:nearest.y}}const length=path.getTotalLength();let low=0,high=length,point=path.getPointAtLength(0);for(let i=0;i<18;i++){const mid=(low+high)/2;point=path.getPointAtLength(mid);if(point.x<x)low=mid;else high=mid}return path.getPointAtLength((low+high)/2)};
    const paint=()=>{frame=null;if(pendingX==null)return;const x=Math.max(points[0].x,Math.min(points[points.length-1].x,pendingX)),curve=curvePointForX(x),nearest=points.reduce((best,p)=>Math.abs(p.x-x)<Math.abs(best.x-x)?p:best,points[0]),tooltipX=Math.max(60,Math.min(700,curve.x)),tooltipY=Math.max(58,curve.y);guide.setAttribute('x1',curve.x);guide.setAttribute('x2',curve.x);dot.setAttribute('cx',curve.x);dot.setAttribute('cy',curve.y);tooltip.setAttribute('transform',`translate(${tooltipX} ${tooltipY})`);label.textContent=nearest.label;value.textContent=formatMoney(nearest.value,activeBusiness().currency);scrubber.classList.add('active')};
    ...
    svg.addEventListener('pointerdown',event=>{tracking=true;moved=false;startClientX=event.clientX;clearTimeout(hideTimer);svg.setPointerCapture?.(event.pointerId);queue(event)});
    ...
    svg.addEventListener('pointerup',event=>{tracking=false;svg.releasePointerCapture?.(event.pointerId);if(event.pointerType!=='mouse')hideTimer=setTimeout(()=>scrubber.classList.remove('active'),1400);setTimeout(()=>{moved=false},0)});
    svg.addEventListener('pointercancel',()=>{tracking=false;scrubber.classList.remove('active')});
    svg.addEventListener('pointerleave',event=>{if(!tracking&&event.pointerType==='mouse')scrubber.classList.remove('active')});
    svg.addEventListener('click',event=>{if(!moved)return;event.preventDefault();event.stopPropagation();moved=false},true);
    pointNodes.forEach((node,index)=>node.addEventListener('focus',()=>{pendingX=points[index].x;paint()}));
  }
  ...
  function bindAnalyticsInteractions(bounds) {
    ...
    $$('[data-analytics-pie-kind]').forEach(node=>{const run=()=>{const keys=JSON.parse(decodeURIComponent(node.dataset.analyticsPieKeys));openAnalyticsEvidence(node.dataset.analyticsPieKind==='client'?'clientGroup':'categoryGroup',{bounds,clientKeys:keys,categoryKeys:keys})};node.addEventListener('click',run);node.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();run()}})});
