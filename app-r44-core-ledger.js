(() => {
  ...
  function openSubfilterMenu(anchor, { kind, title, items, currentValue, onSelect }) {
    if (activeFilterMenu?.anchor === anchor) { closeFilterMenu(); return; }
    closeFilterMenu();
    const menu = document.createElement('div');
    menu.className = `filter-popover subfilter-popover subfilter-${kind}`;
    menu.setAttribute('role', 'menu');
    const allLabel = kind === 'clients' ? 'All clients' : 'All categories';
    const choices = items.map(item => {
      ...
      if (kind === 'clients') {
        ...
        return `<button type="button" role="menuitemradio" aria-checked="${selected}" aria-label="${escapeHtml(item.label)}" title="${escapeHtml(item.label)}" class="subfilter-client-choice ${selected ? 'selected' : ''}" data-subfilter-value="${escapeHtml(item.value)}"><span class="subfilter-client-initial client-bg-${colorKey}" aria-hidden="true">${escapeHtml(initial)}</span><span class="sr-only">${escapeHtml(item.label)}</span></button>`;
      }
      return `<button type="button" role="menuitemradio" aria-checked="${selected}" class="subfilter-category-choice ${selected ? 'selected' : ''}" data-subfilter-value="${escapeHtml(item.value)}"><span>${escapeHtml(item.label)}</span><span aria-hidden="true">${selected ? '✓' : ''}</span></button>`;
    }).join('');
    menu.innerHTML = `<div class="subfilter-heading"><span>${escapeHtml(title)}</span><button type="button" class="subfilter-clear ${currentValue === 'all' ? 'selected' : ''}" data-subfilter-value="all">${allLabel}</button></div><div class="subfilter-choices">${choices}</div>`;
    document.body.appendChild(menu);
    anchor.setAttribute('aria-expanded', 'true');
    activeFilterMenu = { anchor, menu };
  ...
  function renderSessions() {
    ...
    if (ui.sessionClientFilter !== 'all' && !availableClients.some(client => client.id === ui.sessionClientFilter)) ui.sessionClientFilter = 'all';
    const selectedSessionClient = ui.sessionClientFilter === 'all' ? null : clientById(ui.sessionClientFilter);
    syncSubfilterTrigger($('#sessionClientSubfilterBtn'), { active:!!selectedSessionClient, label:selectedSessionClient?.displayName || '', colorKey:clientColorKey(selectedSessionClient) });
    ...
    const sessions = allSessions.filter(s => {
      ...
      const matchesClient = ui.sessionClientFilter === 'all' || s.clientId === ui.sessionClientFilter;
  ...
  function bindEmptyActions() {
    $$('[data-empty-action]').forEach(btn => btn.addEventListener('click', () => {
      ...
      if (btn.dataset.emptyAction === 'clear-session-filters') { ui.sessionFilter = 'all'; ui.sessionClientFilter = 'all'; ui.sessionPage = 1; if ($('#sessionSearch')) $('#sessionSearch').value = ''; renderSessions(); return; }
  ...
  function renderMoney() {
    ...
    const visibleExpenses = expenses.filter(expense => {
      ...
      const matchesCategory = ui.expenseCategoryFilter === 'all' || expense.category === ui.expenseCategoryFilter;
    ...
    const selectedExpenseCategory = ui.expenseCategoryFilter === 'all' ? null : expenseCategoryLabel(ui.expenseCategoryFilter);
    syncSubfilterTrigger($('#expenseCategorySubfilterBtn'), { active:!!selectedExpenseCategory, label:selectedExpenseCategory || '', category:true });
  ...
  function bindMoneyEmptyActions() {
    ...
    $$('[data-empty-action="all-expenses"]').forEach(btn => btn.addEventListener('click', () => { ui.expenseFilter = 'all'; ui.expenseCategoryFilter = 'all'; ui.expensePage = 1; renderMoney(); }));
  }
  ...
  $('#sessionClientSubfilterBtn').addEventListener('click', event => openSubfilterMenu(event.currentTarget, {
    ...
    currentValue:ui.sessionClientFilter,
    onSelect:value => { ui.sessionClientFilter = value; ui.sessionPage = 1; renderSessions(); }
  }));
  ...
  $('#expenseCategorySubfilterBtn').addEventListener('click', event => openSubfilterMenu(event.currentTarget, {
    ...
    currentValue:ui.expenseCategoryFilter,
    onSelect:value => { ui.expenseCategoryFilter = value; ui.expensePage = 1; renderMoney(); }
  }));
