(() => {
  ...
  function analyticsExpenseCategoryChart(buckets,expenses) {
    ...
    return `<svg class="analytics-svg category-stack-svg category-column-chart" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Business-use expenses shown as continuous monthly columns stacked by category."><defs>${gradientDefs}${columnClipDefs}</defs>${frame.grid}${bars}</svg><div class="category-stack-legend" aria-label="Expense categories">${legend}</div>`;
  }
