(() => {
  ...
  function analyticsRecordYears() {
    ...
    const years=[...analyticsPaymentsForReview().map(x=>x.receivedDate),...analyticsExpensesForReview().map(x=>x.date),...businessSessions().map(x=>x.date),...businessInvoices().map(x=>x.issueDate)].filter(Boolean).map(value=>Number(value.slice(0,4))).filter(Number.isFinite);
  ...
  function analyticsMonthBuckets(bounds) {
    ...
    analyticsExpensesForReview().filter(x=>dateInBounds(x.date,bounds.start,bounds.end)).forEach(x=>{const b=byKey.get(x.date.slice(0,7));if(b)b.expenses+=Number(x.businessCents||0)});
  ...
  const MOSAIC_BLOCK_CENTS=25000;
  const MOSAIC_CATEGORY_ORDER=['food','gas','parking','car','subscriptions','misc'];
  const MOSAIC_CATEGORY_COLORS={food:'#f0b64f',gas:'#4da8ef',parking:'#64cf83',car:'#ef7667',subscriptions:'#a56ee8',misc:'#c3d5df'};
  ...
  const ANALYTICS_REVIEW_MONTHS=[
    {month:'2026-04',income:320000,expenses:{food:70000,gas:50000,parking:25000,car:50000,subscriptions:25000,misc:25000}},
    {month:'2026-05',income:400000,expenses:{food:77500,gas:60000,parking:30000,car:60000,subscriptions:50000,misc:40000}},
    {month:'2026-06',income:450000,expenses:{food:95000,gas:80000,parking:60000,car:75000,subscriptions:60000,misc:50000}},
    {month:'2026-07',income:520000,expenses:{food:120000,gas:65000,parking:70000,car:80000,subscriptions:65000,misc:75000}},
    {month:'2026-08',income:610000,expenses:{food:95000,gas:65000,parking:50000,car:75000,subscriptions:40000,misc:35000}},
    {month:'2026-09',income:570000,expenses:{food:50000,gas:55000,parking:50000,car:65000,subscriptions:32500,misc:40000}}
  ];

  function analyticsReviewSampleData() {
    if(!ANALYTICS_REVIEW_SAMPLE_DATA||document.body?.dataset.analyticsSample!=='true')return {payments:[],expenses:[]};
    const businessId=activeBusiness().id;
    const payments=ANALYTICS_REVIEW_MONTHS.map((row,index)=>({id:`analytics-review-payment-${index}`,businessId,kind:'direct',sourceName:'Sample income',receivedDate:`${row.month}-15`,amountCents:row.income,method:'sample',analyticsReviewSample:true}));
    const expenses=ANALYTICS_REVIEW_MONTHS.flatMap((row,monthIndex)=>Object.entries(row.expenses).map(([category,businessCents],categoryIndex)=>({id:`analytics-review-expense-${monthIndex}-${categoryIndex}`,businessId,merchant:`Sample ${expenseCategoryLabel(category)}`,date:`${row.month}-${String(6+categoryIndex*3).padStart(2,'0')}`,totalCents:businessCents,businessCents,category,classification:'business',reviewStatus:'ready',analyticsReviewSample:true})));
  ...
  function analyticsExpenseCategoryChart(buckets,expenses) {
    ...
    const ranked=[...categoryTotals].sort((a,b)=>{const aIndex=MOSAIC_CATEGORY_ORDER.indexOf(a[0]),bIndex=MOSAIC_CATEGORY_ORDER.indexOf(b[0]);if(aIndex>=0||bIndex>=0)return (aIndex<0?999:aIndex)-(bIndex<0?999:bIndex);return b[1]-a[1]});
    ...
    const series=leading.map(([key,total],index)=>({key,label:expenseCategoryLabel(key),total,color:MOSAIC_CATEGORY_COLORS[key]||ANALYTICS_PIE_COLORS[(index+6)%ANALYTICS_PIE_COLORS.length]}));
    ...
    const largestTotal=Math.max(0,...totals);const blockScaleCeiling=Math.max(MOSAIC_BLOCK_CENTS,Math.ceil(largestTotal/MOSAIC_BLOCK_CENTS)*MOSAIC_BLOCK_CENTS);
    const frame=analyticsChartFrame(buckets,[...totals,blockScaleCeiling],{height:350});const baseline=frame.y(0);const slot=frame.innerW/Math.max(1,buckets.length);const barWidth=Math.min(66,Math.max(31,slot*.58));
    const gradientDefs=series.map((item,index)=>`<linearGradient id="mosaicFill${index}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${item.color}" stop-opacity=".98"/><stop offset=".5" stop-color="${item.color}" stop-opacity=".78"/><stop offset="1" stop-color="${item.color}" stop-opacity=".94"/></linearGradient>`).join('');
    const bars=buckets.map((bucket,index)=>{
      const ranges=[];let cursor=0;
      series.forEach((item,seriesIndex)=>{const value=monthValues[index].get(item.key)||0;if(value>0){ranges.push({item,seriesIndex,start:cursor,end:cursor+value});cursor+=value}});
      const unitCount=Math.ceil(totals[index]/MOSAIC_BLOCK_CENTS);
      const towerTop=unitCount?frame.y(unitCount*MOSAIC_BLOCK_CENTS):baseline;
      const units=Array.from({length:unitCount},(_,unitIndex)=>{
        const unitStart=unitIndex*MOSAIC_BLOCK_CENTS,unitEnd=unitStart+MOSAIC_BLOCK_CENTS;
        const shellTop=frame.y(unitEnd),shellBottom=frame.y(unitStart),shellHeight=shellBottom-shellTop;
        const fills=ranges.map(range=>{const start=Math.max(unitStart,range.start),end=Math.min(unitEnd,range.end);if(end<=start)return '';const rawTop=frame.y(end),rawBottom=frame.y(start);return `<rect class="mosaic-fragment" data-fragment-cents="${end-start}" x="${frame.x(index)-barWidth/2+2.3}" y="${rawTop+1.65}" width="${barWidth-4.6}" height="${Math.max(.7,rawBottom-rawTop-3.3)}" rx="3.5" fill="url(#mosaicFill${range.seriesIndex})" style="--mosaic-color:${range.item.color}"><title>${escapeHtml(bucket.label)} · ${escapeHtml(range.item.label)} · ${formatMoney(end-start)}</title></rect>`}).join('');
        return `<g class="mosaic-unit" data-unit-index="${unitIndex}">${fills}<rect class="mosaic-unit-highlight" x="${frame.x(index)-barWidth/2+2.3}" y="${shellTop+1.65}" width="${barWidth-4.6}" height="${Math.max(.7,shellHeight-3.3)}" rx="3.5"/><rect class="mosaic-unit-shell" data-unit-cents="${MOSAIC_BLOCK_CENTS}" x="${frame.x(index)-barWidth/2}" y="${shellTop+.65}" width="${barWidth}" height="${Math.max(1,shellHeight-1.3)}" rx="4.8"><title>${escapeHtml(bucket.label)} · $${(unitIndex*250).toLocaleString('en-US')}–$${((unitIndex+1)*250).toLocaleString('en-US')} block</title></rect></g>`;
      }).join('');
      const auraHeight=Math.max(0,baseline-towerTop);const totalY=Math.max(14,towerTop-9);
      return `<g class="stacked-month mosaic-month" data-chart-index="${index}" data-chart-x="${frame.x(index)}" data-chart-y="${frame.y(totals[index])}" data-chart-value="${totals[index]}" data-chart-label="${escapeHtml(bucket.label)}">${unitCount?`<rect class="mosaic-column-aura" x="${frame.x(index)-barWidth/2-5}" y="${towerTop-4}" width="${barWidth+10}" height="${auraHeight+8}" rx="8"/>`:''}${units}${totals[index]>0?`<text class="mosaic-total" x="${frame.x(index)}" y="${totalY}" text-anchor="middle">${formatMoney(totals[index])}</text>`:''}<rect class="stacked-hit" x="${frame.x(index)-slot/2}" y="${frame.top}" width="${slot}" height="${frame.innerH}"/>${index%frame.labelEvery===0||index===buckets.length-1?`<text class="mosaic-month-label" x="${frame.x(index)}" y="${frame.height-10}" text-anchor="middle">${escapeHtml(bucket.label)}</text>`:''}</g>`;
    }).join('');
    const legend=series.map(item=>`<span><i style="--category-color:${item.color}"></i>${escapeHtml(item.label)}</span>`).join('');
    return `<svg class="analytics-svg category-stack-svg mosaic-chart-svg scrubbable-chart" data-scrub-mode="stacked" viewBox="0 0 ${frame.width} ${frame.height}" role="img" aria-label="Business-use expenses shown as measured mosaic columns. Each full block represents $250; partial blocks preserve exact amounts. Touch and drag to inspect monthly totals."><defs>${gradientDefs}</defs>${frame.grid}${bars}<g class="chart-scrubber stacked-scrubber" aria-hidden="true"><line x1="0" x2="0" y1="${frame.top}" y2="${frame.top+frame.innerH}"/><circle class="expense" cx="0" cy="${baseline}" r="5.5"/><g class="scrub-tooltip"><rect x="-54" y="-42" width="108" height="32" rx="9"/><text class="scrub-label" x="0" y="-29" text-anchor="middle"></text><text class="scrub-value" x="0" y="-17" text-anchor="middle"></text></g></g></svg><div class="category-stack-legend" aria-label="Expense categories">${legend}</div>`;
  }
